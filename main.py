...
class MangaEditorApp(QMainWindow):
    ...
    def check_and_load_lama(self, prompt=True):
        ...
        if not os.path.exists(model_path):
            if prompt:
                reply = QMessageBox.question(
                    self, 
                    "Загрузка ИИ-модели", 
                    "Для качественной очистки (качество IOPaint один в один) необходимо скачать ИИ-модель LaMa (около 90 МБ).\nСкачать её сейчас?",
                    QMessageBox.Yes | QMessageBox.No
                )
                if reply != QMessageBox.Yes:
                    return False
            
            # Скачиваем с Hugging Face по прямой ссылке
            ok = self.download_model_gui("https://huggingface.co/Carve/LaMa-ONNX/resolve/main/lama_fp32.onnx", model_path)
            if not ok:
        ...
        if self.lama_inpainter is None:
            try:
                self.statusBar().showMessage("Загрузка ИИ-модели LaМа в память...")
                QApplication.processEvents()
                self.lama_inpainter = LaMaInpainter(model_path)
                self.statusBar().showMessage("ИИ-модель LaМа готова к работе!", 3000)
            except Exception as e:
